  ...
  return (
    <tr>
      ...
      <td style={{ minWidth: 190 }}>
        {canWrite ? <select aria-label={`${risk.title} ${t("risks.owner")}`} value={owner} onChange={(event) => setOwner(event.target.value)}><option value="">{t("risks.unassigned")}</option>{owners.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : ownerName}
